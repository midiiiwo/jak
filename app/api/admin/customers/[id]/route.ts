...
export async function GET(
    ...
    try {
        ...
        const orderHistory = mockOrderHistory.filter(order => order.customerId === customerId);
        ...
        const metrics = {
            ...
            daysSinceLastOrder: customer.lastOrderDate
                ...
                : null,
            daysSinceRegistration: Math.floor((new Date().getTime() - new Date(customer.registrationDate).getTime()) / (1000 * 60 * 60 * 24)),
            monthlySpending: customer.totalSpent / Math.max(1, Math.ceil(
                (new Date().getTime() - new Date(customer.registrationDate).getTime()) / (1000 * 60 * 60 * 24 * 30)
            ))
        };
...
export async function DELETE(
    ...
    try {
        ...
        const pendingOrders = mockOrderHistory.filter(
            order => order.customerId === customerId && order.status === 'processing'
        );
